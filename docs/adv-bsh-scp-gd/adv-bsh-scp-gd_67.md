# 附录 M. 样本 .bashrc 和 .bash_profile 文件

> 原文：[`tldp.org/LDP/abs/html/sample-bashrc.html`](https://tldp.org/LDP/abs/html/sample-bashrc.html)

`~/.bashrc`文件决定了交互式 shell 的行为。仔细查看这个文件可以更好地理解 Bash。

Emmanuel Rouat 贡献了以下非常详尽的`.bashrc`文件，该文件是为 Linux 系统编写的。他欢迎读者对此提出反馈。

仔细研究这个文件，并在您的`.bashrc`文件或甚至脚本中自由地重用代码片段和函数。
...
此外，这里是从 Andrzej Szelachowski 的指导性`.bash_profile`文件中摘录的一段内容。

**示例 M-2\. `.bash_profile` 文件**
