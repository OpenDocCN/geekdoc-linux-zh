...
If given several URLs and a transfer completes faster than the allowed rate, curl delays starting the next transfer to maintain the requested rate. This option is for serial transfers and has no effect when [–parallel](ch066.xhtml#cmdline__urls__parallel__md) is used.
...
When retrying transfers, enabled with [–retry](ch095.xhtml#usingcurl__downloads__retry__md), the separate retry delay logic is used and not this setting.
