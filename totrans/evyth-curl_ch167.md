...
Previous chapters talked about [regular POST](ch154.xhtml#http__post__simple__md) and [multipart formpost](ch165.xhtml#http__post__multipart__md), and in your typical command lines you do them with `-d` or `-F`.
...
If these services expect plain raw data or perhaps data formatted as JSON or similar, you want the [regular POST](ch154.xhtml#http__post__simple__md) approach. curl’s `-d` option does not alter or encode the data at all but just sends exactly what you tell it to. Just pay attention that `-d` sets a default `Content-Type:` that might not be what you want.