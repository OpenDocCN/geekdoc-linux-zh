...
One way to avoid passing the username and password on the command line is to instead use a [.netrc file](ch109.xhtml#usingcurl__netrc__md) or a [config file](ch070.xhtml#cmdline__configfile__md). You can also use the `-u` option without specifying the password, and then curl instead prompts the user for it when it runs.
