...
All easy handles added to the same multi handle automatically share [connection cache](ch233.xhtml#transfers__conn__reuse__md) and [dns cache](ch234.xhtml#transfers__conn__names__md).
...
`CURL_LOCK_DATA_DNS` - the DNS cache is where libcurl stores addresses for resolved hostnames for a while to make subsequent lookups faster.
