...
It can decide to use the regular [write callback](ch213.xhtml#transfers__callbacks__write__md) to receive incoming data, and respond to that data in or outside of the callback with `curl_ws_send`. Thereby treating the entire session as a form of download from the server.
...
If not using [raw mode](ch273.xhtml#helpers__ws__options__md), libcurl automatically responds with the appropriate `PONG` response for incoming `PING` frames and does not expose them in the API.