...
The connection timeout only limits the time curl is allowed to spend up until the moment it connects, so once the TCP connection has been established it can take longer time. See the [Timeouts](ch108.xhtml#usingcurl__timeouts__md) section for more on generic curl timeouts.
